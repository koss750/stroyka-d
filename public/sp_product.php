...
<select name="baseType">
  ...
  <div class="row">
    <div class="col-md-6">
      <!-- House Image -->
      <img src="path-to-your-house-image.jpg" alt="House" class="img-fluid">
    </div>
    <div class="col-md-6">
      <!-- Selection Form with Checkboxes -->
<div class="container mt-3">
  <!-- House Specifications Title -->
  <h2 class="mb-4">Баня 7,9 на 9,4 "ДОБРАЯ"</h2>
  
  <div class="row flex flex-param">
                    <div class="flex">
                                                    <div class="param">
                                Площадь:
                                <span>67.24 М</span>
  ...
  <!-- Form for Construction Options -->
  <div class="col-md-6">
  <div class="container">
  <h2>Выберите опции для вашего заказа:</h2>
  
  <!-- Фундамент -->
  <div class="mb-3">
    <h4>Фундамент</h4>
    <button type="button" class="btn btn-outline-primary category-button" data-group="foundation">CBC</button>
    <button type="button" class="btn btn-outline-primary category-button" data-group="foundation">ЖБ</button>
    <button type="button" class="btn btn-outline-primary category-button" data-group="foundation">Ленточный</button>
    <div id="foundationOptions" class="options-container d-none"></div>
  </div>
  
  <!-- Домокомплект -->
  <div class="mb-3">
    <h4>Домокомплект</h4>
    <button type="button" class="btn btn-outline-primary category-button" data-group="housekit">ОЦБ Сосна/Ель</button>
    <button type="button" class="btn btn-outline-primary category-button" data-group="housekit">ОЦБ Лиственница</button>
    <!-- Add more buttons for each house kit option -->
    <div id="housekitOptions" class="options-container d-none"></div>
  </div>
  
  <!-- Кровля -->
  <div class="mb-3">
    <h4>Кровля</h4>
    <button type="button" class="btn btn-outline-primary category-button" data-group="roofing">Кровля</button>
    <div id="roofingOptions" class="options-container d-none"></div>
  </div>
...
<script>
  // Define the sizes for each option based on the provided categories
  const options = {
    'foundation': {
      'CBC': ['108х2500', '108х2000'],
      'ЖБ': ['150х150х3000', '200х200х3000'],
      'Ленточный': [
        '300x600', '300x700', '300x800', '300x900', '300x1000',
        '400x600', '400x700', '400x800', '400x900', '400x1000',
        '500x700', '500x800', '500x900', '500x1000',
        '600x700', '600x800', '600x900', '600x1000'
      ]
    },
    // Define options for 'housekit' and 'roofing' here...
  };

  // JavaScript to handle category button click and size selection behavior
  document.querySelectorAll('.category-button').forEach(button => {
    button.addEventListener('click', function() {
      const group = this.dataset.group;
      const category = this.textContent;
      const sizes = options[group][category];
      
      // Toggle category button appearance
      document.querySelectorAll(`.category-button[data-group="${group}"]`).forEach(btn => {
        btn.classList.remove('btn-primary');
      });
      this.classList.add('btn-primary');
      
      // Generate size buttons and show them
      const optionsContainer = document.querySelector(`#${group}Options`);
      ...
      optionsContainer.querySelectorAll('.size-button').forEach(sizeBtn => {
        sizeBtn.addEventListener('click', function() {
          this.classList.toggle('btn-secondary');
          this.classList.toggle('btn-info'); // 'btn-info' is a Bootstrap class for a shade of blue
        });
      });
    });
  });
