                    ...
                    if (in_array($sheetname, $this->exceptionalSheetsArray)) {
                        ...
                        if ($specialArray != null) {
                            foreach ($specialArray as $invoiceId => $result) {
                                ...
                                $results[$design->id][$invoiceId]['parameters'] = $result['parameters'];
                            }
                        }
                    } else {
    ...
    protected function getSheetParameters($sheetname, $design)
{
    ...
    foreach ($sheetStructure['sections'] as $sectionKey => &$section) {
        ...
        foreach ($section['labourItems'] as &$labourItem) {
            $labourItem['labourPrice'] = $this->getCellValue($sheet, $labourItem['labourPriceCell']);
            $labourItem['labourQuantity'] = $this->getCellValue($sheet, $labourItem['labourQuantityCell']);
            $labourItem['labourTotal'] = $this->getCellValue($sheet, $labourItem['labourTotalCell']);
            $labourTotal += $labourItem['labourTotal'];
        }

        foreach ($section['materialItems'] as &$materialItem) {
            $materialItem['materialPrice'] = $this->getCellValue($sheet, $materialItem['materialPriceCell']);
            $materialItem['materialQuantity'] = $this->getCellValue($sheet, $materialItem['materialQuantityCell']);
            $materialItem['materialTotal'] = $this->getCellValue($sheet, $materialItem['materialTotalCell']);
            $materialTotal += $materialItem['materialTotal'];
        }

        $section['sectionTotalValue'] = $labourTotal + $materialTotal;
